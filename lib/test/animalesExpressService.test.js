...
test('responde preguntas frecuentes sin depender del proveedor de IA', () => {
  assert.match(answerKnownFaq('¿Hacéis envíos?'), /transporte de mascotas y otros animales/i)
  assert.match(
    answerKnownFaq('Hola, explícame cómo funciona eso del transporte'),
    /código postal y población de recogida/i
  )
  assert.match(answerKnownFaq('¿Tienes patatillas?'), /No vendemos ni enviamos productos/i)
  assert.match(answerKnownFaq('¿Cuánto cuesta?'), /desde \*50 €\*/i)
  assert.equal(answerKnownFaq('Cuéntame algo creativo'), '')
})
...
test('aplica temperatura baja, cola y exclusiones configurables', () => {
  ...
  process.env.AE_AI_TIMEOUT_MS = '20000'
  process.env.AE_AI_MAX_RETRIES = '1'
  ...
  try {
    ...
    assert.equal(config.aiTimeoutMs, 20000)
    assert.equal(config.aiMaxRetries, 1)
