...
const enqueueZaiRequest = (request) => {
  const queued = zaiRequestQueue.then(request, request)
...
const callLmStudio = async (system, question, history = []) => {
  ...
  const response = /api\.z\.ai/i.test(config.lmStudioUrl)
    ? await enqueueZaiRequest(() => requestWithRetry(request))
    : await request()
...
const answerPublicQuestion = async (question, history = []) => {
  const config = getConfig()
...
${routeAnalysis || 'No se han identificado claramente dos paradas distintas. Solicita origen y destino.'}`

  return callLmStudio(system, question, history)
}
...
const appendLead = async ({ jid, pickupPostalCode, deliveryPostalCode, animals, originalMessage }) => {
  const config = getConfig()
  const sheets = await getSheetsClient()
