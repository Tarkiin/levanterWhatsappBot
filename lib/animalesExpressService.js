...
const requestWithRetry = async (request, maxRetries = 1) => {
  const delays = [2000, 5000, 10000]
  const retryCount = Math.min(Math.max(0, Math.floor(maxRetries)), delays.length)
  for (let attempt = 0; ; attempt += 1) {
    try {
      ...
      console.warn(
        `[AnimalesExpress] IA temporalmente ocupada; reintento ${attempt + 1}/${retryCount} en ${delays[attempt]} ms (${describeAiError(error)})`
      )
...
const getConfig = () => ({
  ...
  aiTemperature: numberValue(process.env.AE_AI_TEMPERATURE, 0.2, 0, 1),
  aiMinIntervalMs: numberValue(process.env.AE_AI_MIN_INTERVAL_MS, 5000, 0, 60000),
  aiTimeoutMs: numberValue(process.env.AE_AI_TIMEOUT_MS, 20000, 5000, 120000),
  aiMaxRetries: numberValue(process.env.AE_AI_MAX_RETRIES, 1, 0, 3),
...
const callLmStudio = async (system, question, history = []) => {
  const config = getConfig()
  if (/api\.z\.ai/i.test(config.lmStudioUrl) && !config.lmStudioApiKey) {
    throw new Error('Falta AE_LM_STUDIO_API_KEY para usar Z.AI')
  }
  ...
  const request = () =>
    axios.post(
      config.lmStudioUrl,
      payload,
      // Un proveedor bloqueado no debe retener durante minutos la cola privada.
      { headers, timeout: config.aiTimeoutMs }
    )
  ...
  const response = /api\.z\.ai/i.test(config.lmStudioUrl)
    ? await requestWithRetry(
        // Cada reintento vuelve a la cola para no convertir un 5xx en un 429.
        () => enqueueZaiRequest(request, config.aiMinIntervalMs),
        config.aiMaxRetries
      )
    : await request()
...
const answerKnownFaq = (text = '') => {
  const question = normalize(text)
  if (!question) return ''

  if (
    /\b(?:patata|patatas|patatilla|patatillas|comida|alimento|paquete|paquetes|mueble|muebles|mercancia|mercancias)\b/.test(
      question
    )
  ) {
    return 'No vendemos ni enviamos productos. AnimalesExpress está especializado en el transporte de animales.'
  }

  if (/\b(?:como funciona|como trabajais|como es el transporte|explicame).*\b(?:transporte|servicio|envio)\b/.test(question)) {
    return 'Transportamos animales de forma segura y coordinada. Para estudiar un servicio necesitamos el código postal y población de recogida, el código postal y población de entrega, la cantidad y especie de animales y una fecha aproximada. El equipo confirma después la viabilidad, la fecha y el precio.'
  }

  if (
    /\b(?:haceis|realizais|ofreceis|teneis)\b.*\b(?:envio|envios|transporte|transportes|traslado|traslados)\b/.test(
      question
    ) || /^(?:hola )?(?:haceis )?envios?(?: de (?:mascotas|animales))?$/.test(question)
  ) {
    return 'Sí. AnimalesExpress realiza transporte de mascotas y otros animales, tanto nacional como internacional, sujeto a que el equipo confirme la viabilidad del trayecto.'
  }

  if (/\b(?:precio|precios|cuanto cuesta|cuanto vale|tarifa|tarifas)\b/.test(question)) {
    return 'Los portes son desde *50 €*. El precio final depende del trayecto y de los animales, y lo confirma el equipo después de recibir los datos de recogida y entrega.'
  }

  if (/\b(?:internacional|extranjero|fuera de espana)\b/.test(question)) {
    return 'Sí, se realizan transportes nacionales e internacionales, siempre sujetos a confirmación del equipo y a los requisitos legales del trayecto.'
  }

  if (/\b(?:que animales|que mascotas|especies|perros|gatos|aves|reptiles)\b/.test(question)) {
    return 'Transportamos perros, gatos, aves, reptiles y otras especies especiales. El equipo debe confirmar cada servicio según la especie, el trayecto y sus necesidades.'
  }
