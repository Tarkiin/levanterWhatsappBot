...
const getConfig = () => ({
  ...
  lmStudioFallbackModel:
    process.env.AE_LM_STUDIO_FALLBACK_MODEL ||
    (/api\.z\.ai/i.test(process.env.AE_LM_STUDIO_URL || '') ? 'glm-4.5-flash' : ''),
  ...
  aiTimeoutMs: numberValue(process.env.AE_AI_TIMEOUT_MS, 15000, 5000, 120000),
  aiMaxRetries: numberValue(process.env.AE_AI_MAX_RETRIES, 0, 0, 3),
...
const callLmStudio = async (system, question, history = []) => {
  ...
  const models = [config.lmStudioModel, config.lmStudioFallbackModel].filter(
  ...
  for (let index = 0; index < models.length; index += 1) {
    ...
    try {
      response = /api\.z\.ai/i.test(config.lmStudioUrl)
        ? await requestWithRetry(
            // Cada intento vuelve a la cola para respetar el límite global.
            () => enqueueZaiRequest(() => request(model), config.aiMinIntervalMs),
            config.aiMaxRetries
          )
        : await request(model)
      break
      ...
      if (nextModel) {
        console.warn(
          `[AnimalesExpress] El modelo ${model} no respondió; probando ${nextModel} (${error.message})`
        )
      }
    }
  }

  if (!response) throw lastError || new Error('La IA no devolvió una respuesta')
...
- Nunca pidas todos esos datos juntos. Si corresponde continuar una solicitud, solicita como máximo un dato cada vez; el flujo del bot los recopila paso a paso con ejemplos.
...
- Destaca con *negrita* el nombre de la empresa, los datos importantes y los pasos clave; no escribas toda la respuesta en negrita.
- Usa uno o dos emojis cuando encajen de forma natural, pero no en todas las respuestas ni siempre los mismos.
- Habla siempre como asistente virtual de AnimalesExpress, con un tono cercano y profesional.
- La aplicación añade la presentación del asistente cuando corresponde; no repitas esa presentación literalmente dentro de tu respuesta.
${
  options.privateMode
